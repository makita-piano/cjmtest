<template>
  <div class="grid-center">
    <div class="col-9_sm-12" style="max-width: 620px;padding: 0 1rem">
      <div>
        <h1>オール・ショパン・リサイタル</h1>
      </div>
      <div class="grid-center col-12">
        <h2 class="col-12">公演日時</h2>
        <div class="col-12">
          <ul class="pre-list">
            <li><p>・2021/12/11(土)</p></li>
            <li><p>・14：00開演(13:30開場)</p></li>
            <li><p>・会場：ヤマハホール(東京・銀座)</p></li>
            <li><p>・出演：菊地裕介</p></li>
            <br>
        ...
        <div class="col-6_sm-12" style="margin-top: 1rem;padding: .5rem" @click="openModal(0)">
          <nuxt-img src="/img/pos3.png" width="768" style="width: 100%;max-width: 768px;cursor: pointer" />
        </div>
        <div class="col-6_sm-12" style="margin-top: 1rem;padding: .5rem" @click="openModal(1)" >
          <nuxt-img src="/img/pos4.png" width="768" style="width: 100%;max-width: 768px;cursor: pointer" />
        </div>
        ...
        <div v-else class="grid col-12">
          ...
          <div class="col-6_sm-12" @click="openModal(2)" >
            <nuxt-img src="/img/yamaha_1.png" width="768" style="width: 100%;cursor: pointer" />
          </div>
          <div class="col-6_sm-12" @click="openModal(3)">
            <nuxt-img src="/img/yamaha_2.png" width="768" style="width: 100%;cursor: pointer" />
          </div>
          <div class="grid col-12">
            ...
            <div v-for="key in all_seats_keys()" v-on:click="col_click(key)" class="col-3_sm-6" :class="['select-col-' + key, {active: click_col === key}]" :key="key">
              <div style="border: 1px solid #999;padding: 1rem;margin: .5rem;cursor: pointer">
          ...
          <div class="grid col-12" style="margin-top: 2rem">
            <div v-if="input_check" v-on:click="submit_click()" class="btn" style="margin-bottom: 3rem;cursor: pointer">
              <p>進む</p>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    head() {
      return {
        title: "オール・ショパン・リサイタル｜神宮の杜音楽院",
        meta: [
          { hid: 'description', name: 'description', content: '2021/12/11(土)14：00開演 コロナ禍の下でベートーヴェンのソロ全曲演奏を終えましたが、ちょうど娘の受験などもありしばらく演奏活動を...' },
          { hid: 'og:description', name: 'og:description', content: '2021/12/11(土)14：00開演 コロナ禍の下でベートーヴェンのソロ全曲演奏を終えましたが、ちょうど娘の受験などもありしばらく演奏活動を...' },
          { hid: 'og:title', property: 'og:title', content: 'オール・ショパン・リサイタル｜神宮の杜音楽院' },
          { hid: 'og:image', property: 'og:image', content: 'https://cjmtokyo.com/img/pos3.png' },
        ]
      }
    },
    data () {
      return {
        ...
        modal: false,
        item_json: [
          {id: 0, url: '/img/pos3.png'},
          {id: 1, url: '/img/pos4.png'},
          {id: 2, url: '/img/yamaha_1.png'},
          {id: 3, url: '/img/yamaha_2.png'},
        ],
        modal_url: '',
        close: (new Date()).getTime() > (new Date(2021, 11, 11, 0, 0, 0)).getTime()
      }
    },
    created(){
      axios
        .get(process.env.VUE_APP_MICROCMS_URL + "/seat2?filters=not_display[equals]false[and]buyer[not_exists]&fields=id,col,row&limit=300",{
      ...
      axios
        .get(process.env.VUE_APP_MICROCMS_URL + "/seat2?filters=not_display[equals]false&fields=col,row&limit=300",{
          headers: { "X-MICROCMS-API-KEY": process.env.VUE_APP_MICROCMS_KEY},
          data: {}
      })
        .then(v => {
    ...
    methods: {
      ...
      submit_click: function(){
        ...
        axios
          .post(process.env.VUE_APP_SLACK_URL2,
        ...
        axios
          .patch(process.env.VUE_APP_MICROCMS_URL + "/seat2/" + id,
          ...
          .then(v => {
            window.location.href = 'https://docs.google.com/forms/d/e/1FAIpQLScQRJPhKNOZMhRH0orjkB7y8AZ48BcWYLc9o6X4rwO-b2XtCA/viewform';
          }).catch(err => {
          });
      },
      openModal(number){
